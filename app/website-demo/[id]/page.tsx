...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  return {
    title: page?.meta_title || page?.name || 'Your Free Website Concept | Nexorra',
    description: page?.meta_description || 'A custom website concept built just for your business.',
    robots: 'noindex',
  };
}

export default async function WebsiteDemoPage({ params }: { params: { id: string } }) {
  void headers();
  const { data: page } = await supabaseAdmin
    .from('landing_pages')
