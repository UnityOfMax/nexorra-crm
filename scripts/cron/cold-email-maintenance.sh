...
AGENT_ID="cold-email-maintenance"
...
START=$(date +%s)
echo "$(date): Starting cold-email-maintenance (run: $RUN_ID)" >> "$LOG_FILE"

claude -p "$(cat .claude/commands/nexorra/cold-email-maintenance.md)" \
  --model haiku \
...
echo "$(date): Finished cold-email-maintenance ($STATUS, ${DURATION}s)" >> "$LOG_FILE"
