...
async function generateDailyReport() {
  const reportDate = "2026-04-05";
  const today = new Date(reportDate + "T00:00:00Z");
  const tomorrow = new Date(new Date(reportDate + "T00:00:00Z").getTime() + 86400000);
  const todayStart = today.toISOString();
  const todayEnd = tomorrow.toISOString();

  const yesterday = new Date(new Date(reportDate + "T00:00:00Z").getTime() - 86400000);
  const last7Days = new Date(new Date(reportDate + "T00:00:00Z").getTime() - 7 * 86400000);
  ...
  report += `\n**Summary:** ${accountCount || 0} active accounts | ${totalNewContacts} new contacts | ${totalMessages} messages | ${totalDeals} active deals\n\n`;
...
- **Client accounts:** ${accountCount || 0} active
