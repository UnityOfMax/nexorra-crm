#!/bin/bash
# Launch Chrome for Petra GMB workers.
# Usage: bash chrome-launch-petra.sh [PORT]  (defaults to 9232)
# Safe to call repeatedly — exits immediately if port is already up.

set -euo pipefail

PORT=${1:-9232}
DEBUG_PROFILE="/home/max/.config/chrome-petra-${PORT}"
LOG="/home/max/crm/logs/chrome-petra-${PORT}.log"

if curl -s --connect-timeout 5 "http://localhost:${PORT}/json/version" > /dev/null 2>&1; then
  echo "Chrome debug port ${PORT} already up."
  exit 0
fi

# Kill any orphaned Chrome processes for this profile before launching fresh
pkill -9 -f "chrome-petra-${PORT}" 2>/dev/null || true
sleep 1

CHROME=""
for cmd in google-chrome google-chrome-stable chromium chromium-browser; do
  if command -v "$cmd" > /dev/null 2>&1; then CHROME="$cmd"; break; fi
done

if [ -z "$CHROME" ]; then
  echo "[$(date)] ERROR: Chrome binary not found on $(hostname)." | tee -a "$LOG"
  exit 1
fi

mkdir -p "$DEBUG_PROFILE" "$(dirname "$LOG")"
echo "[$(date)] Launching Chrome Petra instance on port ${PORT}..." | tee -a "$LOG"

resolve_display() {
  unset WAYLAND_DISPLAY
  export XDG_RUNTIME_DIR="/run/user/1000"

  for _i in $(seq 1 12); do
    if DISPLAY=":99" xdpyinfo >/dev/null 2>&1; then
      export DISPLAY=":99"
      export XAUTHORITY=""
      return 0
    fi
    [ "$_i" -eq 1 ] && bash /home/max/crm/scripts/setup/start-xvfb.sh >/dev/null 2>&1 &
    sleep 5
  done

  XAUTH_CANDIDATE=""
  for i in $(seq 1 6); do
    XAUTH_CANDIDATE=$(ls -t /run/user/1000/.mutter-Xwaylandauth.* 2>/dev/null | head -1)
    [ -n "$XAUTH_CANDIDATE" ] && break
    sleep 5
  done

  [ -n "$XAUTH_CANDIDATE" ] && export XAUTHORITY="$XAUTH_CANDIDATE"
  [ -f "${HOME}/.Xauthority" ] && export XAUTHORITY="${HOME}/.Xauthority"

  for disp in ":0" ":1"; do
    if DISPLAY="$disp" xdpyinfo >/dev/null 2>&1; then
      export DISPLAY="$disp"
      return 0
    fi
  done

  [ -n "$XAUTH_CANDIDATE" ] && export DISPLAY=":0" && return 0
  return 1
}

WAITED=0
until resolve_display; do
  if [ $WAITED -ge 60 ]; then
    echo "[$(date)] ERROR: No X11 display after 60s. Giving up." | tee -a "$LOG"
    exit 1
  fi
  echo "[$(date)] Waiting for X11 display... (${WAITED}s)" | tee -a "$LOG"
  sleep 5
  WAITED=$((WAITED + 5))
done

echo "[$(date)] Using DISPLAY=$DISPLAY" | tee -a "$LOG"

mkdir -p "/tmp/chrome-xdg-${PORT}"
DBUS_SESSION_BUS_ADDRESS="" WAYLAND_DISPLAY="" XDG_RUNTIME_DIR="/tmp/chrome-xdg-${PORT}" "$CHROME" \
  --remote-debugging-port=${PORT} \
  --remote-debugging-address=127.0.0.1 \
  --user-data-dir="$DEBUG_PROFILE" \
  --no-first-run \
  --ozone-platform=x11 \
  --disable-gpu \
  --disable-gpu-sandbox \
  --disable-background-timer-throttling \
  --disable-backgrounding-occluded-windows \
  --disable-renderer-backgrounding \
  --disable-dev-shm-usage \
  --disable-blink-features=AutomationControlled \
  --disable-back-forward-cache \
  --window-size=1280,900 \
  "$@" >> "$LOG" 2>&1 &
disown

for i in $(seq 1 30); do
  if curl -s --connect-timeout 2 "http://localhost:${PORT}/json/version" > /dev/null 2>&1; then
    echo "[$(date)] Chrome Petra ready on port ${PORT}." | tee -a "$LOG"
    exit 0
  fi
  sleep 1
done

echo "[$(date)] ERROR: Chrome Petra (port ${PORT}) launched but not responding after 30s." | tee -a "$LOG"
exit 1
