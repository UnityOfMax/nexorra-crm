#!/bin/bash
# Launch Chrome with remote debugging for Instagram outreach (Stacey).
# Port 9225. Safe to call repeatedly — exits if already up.

set -euo pipefail

PORT=9225
DEBUG_PROFILE="/home/max/.config/chrome-instagram"
LOG="/home/max/crm/logs/chrome-instagram.log"
...
if curl -s --connect-timeout 2 "http://localhost:${PORT}/json/version" > /dev/null 2>&1; then
  echo "Chrome Instagram port ${PORT} already up."
  exit 0
fi
...
if [ -z "$CHROME" ]; then
  MSG="⚠️ Stacey Chrome (port ${PORT}) could not start — Chrome binary not found."
...
echo "[$(date)] Launching Chrome Instagram instance on port ${PORT}..." | tee -a "$LOG"
...
until resolve_display; do
  if [ $WAITED -ge 60 ]; then
    MSG="⚠️ Stacey Chrome (port ${PORT}) could not start — no X11 display after 60s."
    echo "[$(date)] ERROR: $MSG" | tee -a "$LOG"
    send_telegram "$MSG"
    exit 1
  fi
...
for i in $(seq 1 30); do
  if curl -s --connect-timeout 2 "http://localhost:${PORT}/json/version" > /dev/null 2>&1; then
    echo "[$(date)] Chrome Instagram ready on port ${PORT}." | tee -a "$LOG"
    exit 0
  fi
  sleep 1
done

MSG="⚠️ Stacey Chrome (port ${PORT}) launched but not responding after 30s."
echo "[$(date)] ERROR: $MSG" | tee -a "$LOG"
send_telegram "$MSG"
exit 1
