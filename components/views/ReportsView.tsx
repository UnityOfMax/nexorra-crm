...
interface OverviewData {
  ...
  landingPages: {
    page_views: number; cta_clicks: number; form_starts: number; form_submits: number;
    conversion: number; sessions: number; hasData: boolean;
  };
  conversations: {
    total: number; ai_sent: number; inbound: number; outbound: number;
    sms: number; email: number; reply_rate: number; hasData: boolean;
  };
  funnel: { stages: Array<{ label: string; count: number }>; hasData: boolean };
}
  ...
  useEffect(() => {
    ...
    if (isAgency) {
      fetch('/api/analytics/overview')
        ...
        .then(d => { setAgencyData(d); setLoading(false); })
  ...
  return (
    <div style={{ padding: '24px 32px 48px', maxWidth: 1480, margin: '0 auto' }} className="nx-pad-mobile">
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          ...
          <div style={{ color: 'var(--ink-3)', fontSize: 13.5, marginTop: 4 }}>
            {isAgency ? 'All client accounts · last 30 days' : 'Marketing performance, AI conversations, funnel'}
          </div>
        </div>
        {!isAgency && (
      ...
      {loading ? (
        ...
        <AgencyAnalytics data={agencyData} />
      ) : (
        <ClientAnalytics data={data} days={days} lp={lp} conv={conv} funnel={funnel} funnelMax={funnelMax} />
      )}
    </div>
  );
}
...
function AgencyAnalytics({ data }: { data: AgencyData | null }) {
  if (!data) return <EmptyState label="analytics" />;

  const clients = data.clients || [];
  const hasMetaData = clients.some(c => c.spend != null && (c.spend || 0) > 0);

  return (
    <>
      {/* KPI row */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 20 }} className="nx-2col-mobile">
        <KPI label="Total Ad Spend" value={data.totalSpend > 0 ? fmt$(data.totalSpend) : '—'} delta={undefined} trend={[]} tone="blue" />
        <KPI label="Total Leads" value={data.totalLeads > 0 ? fmtN(data.totalLeads) : '—'} delta={undefined} trend={[]} tone="violet" />
        <KPI label="Avg CPL" value={data.avgCpl != null ? fmt$(data.avgCpl) : '—'} delta={undefined} trend={[]} tone="green" />
        <KPI label="Avg Booking Rate" value={data.avgBookingRate != null ? `${data.avgBookingRate}%` : '—'} delta={undefined} trend={[]} tone="amber" />
      </div>
      ...
      <Card padding={0} style={{ overflow: 'hidden' }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--line)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: 15, fontWeight: 600 }}>Client performance</div>
            <div style={{ fontSize: 12, color: 'var(--ink-3)', marginTop: 2 }}>Last 30 days · all ad accounts</div>
          </div>
          <div style={{ fontSize: 12, color: hasMetaData ? 'var(--green)' : 'var(--ink-3)' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--line)' }}>
                {['Client', 'Leads', 'Engaged', 'Bookings', 'Booking Rate', 'Ad Spend', 'CPL'].map((h, i) => (
                  <th key={i} style={{
            ...
            <tbody>
              {clients.map((c, i) => (
                <tr key={c.accountId} style={{ borderBottom: i < clients.length - 1 ? '1px solid var(--line)' : 'none' }}>
                  <td style={{ padding: '12px 16px', fontWeight: 500 }}>{c.accountName}</td>
                  <td style={{ padding: '12px 16px', textAlign: 'right', fontFamily: 'Geist Mono, monospace' }}>{c.leads}</td>
                  <td style={{ padding: '12px 16px', textAlign: 'right', fontFamily: 'Geist Mono, monospace', color: 'var(--ink-2)' }}>{c.engaged}</td>
                  <td style={{ padding: '12px 16px', textAlign: 'right', fontFamily: 'Geist Mono, monospace', color: 'var(--green)' }}>{c.bookings}</td>
                  <td style={{ padding: '12px 16px', textAlign: 'right', fontFamily: 'Geist Mono, monospace', color: (c.bookingRate || 0) >= 5 ? 'var(--green)' : 'var(--ink-2)' }}>
                    {c.bookingRate != null ? `${c.bookingRate}%` : '—'}
                  </td>
                  <td style={{ padding: '12px 16px', textAlign: 'right', fontFamily: 'Geist Mono, monospace' }}>
                    {c.spend != null && c.spend > 0 ? fmt$(c.spend) : '—'}
                  </td>
                  <td style={{ padding: '12px 16px', textAlign: 'right', fontFamily: 'Geist Mono, monospace', color: c.cpl != null && c.cpl < 50 ? 'var(--green)' : 'var(--ink-2)' }}>
        ...
        {!hasMetaData && clients.length > 0 && (
          <div style={{ padding: '12px 20px', borderTop: '1px solid var(--line)', fontSize: 12.5, color: 'var(--ink-3)' }}>
            No Meta ad data yet — connects automatically once META_ACCESS_TOKEN and META_AD_ACCOUNT_ID are set per client.
          </div>
        )}
      </Card>
    </>
...
interface ClientAnalyticsProps {
  ...
  days: number;
  lp: OverviewData['landingPages'] | undefined;
  conv: OverviewData['conversations'] | undefined;
  funnel: OverviewData['funnel'] | undefined;
  funnelMax: number;
}

function ClientAnalytics({ data, days, lp, conv, funnel, funnelMax }: ClientAnalyticsProps) {
  ...
  return (
    <>
      {/* KPI row */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 20 }} className="nx-2col-mobile">
        <KPI label="Ad Spend" value={meta?.hasData ? fmt$(meta.spend) : '—'} delta={undefined} trend={[]} tone="blue" />
        <KPI label="Leads (Meta)" value={meta?.hasData ? fmtN(meta.leads) : '—'} delta={undefined} trend={[]} tone="violet" />
        <KPI label="Cost per Lead" value={meta?.hasData && meta.leads > 0 ? fmt$(meta.cpl) : '—'} delta={undefined} trend={[]} tone="green" />
        <KPI label="AI Reply Rate" value={conv?.hasData ? `${conv.reply_rate}%` : '—'} delta={undefined} trend={[]} tone="amber" />
      </div>
          ...
          {funnel?.hasData ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {funnel.stages.map((s, i) => {
                ...
                const cvt = prev && prev > 0 ? (s.count / prev * 100).toFixed(0) : null;
                return (
                  <div key={s.label} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 130, fontSize: 12.5, color: 'var(--ink-2)', flexShrink: 0 }}>{s.label}</div>
                    <div style={{ flex: 1, height: 28, background: 'var(--paper-2)', borderRadius: 6, overflow: 'hidden' }}>
                      <div style={{
                        width: `${Math.max((s.count / funnelMax) * 100, s.count > 0 ? 4 : 0)}%`, height: '100%',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              {[
                ...
                <div key={row.label} style={{
                  ...
                  <span style={{ fontSize: 13, fontFamily: 'Geist Mono, monospace', fontWeight: 600, color: row.highlight ? 'var(--green)' : 'var(--ink)' }}>
                    {row.value}
                  </span>
                </div>
              ))}
                ...
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--line)' }}>
                    {['Ad set', 'Spend', 'Leads', 'CPL'].map((h, i) => (
                      <th key={i} style={{ padding: '6px 8px', fontSize: 11, fontWeight: 500, color: 'var(--ink-3)', textTransform: 'uppercase', letterSpacing: '0.08em', textAlign: i === 0 ? 'left' : 'right' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
            ...
            <>
              ...
              <div style={{ background: 'var(--paper-2)', borderRadius: 10, padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  ...
                  <div style={{ fontSize: 24, fontWeight: 700, fontFamily: 'Geist Mono, monospace', color: lp.conversion >= 10 ? 'var(--green)' : 'var(--ink)' }}>{lp.conversion}%</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: 12, color: 'var(--ink-3)', marginBottom: 4 }}>Form submits / CTA clicks</div>
                  <div style={{ fontSize: 13, fontFamily: 'Geist Mono, monospace', color: 'var(--ink-2)' }}>{lp.form_submits} / {lp.cta_clicks || lp.sessions}</div>
                </div>
              </div>
            </>
